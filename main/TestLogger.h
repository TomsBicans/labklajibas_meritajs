...
test(logging_float_value) {
  ...
  logger.log(x_id,targetDevice, originDevice, semanticValue, numericValue);
...
test(clear_logs) {
  ...
  uint32_t x_id = millis();
  logger.log(x_id,1, 1, SENSOR_HUMIDITY, static_cast<float>(3.14));
  logger.log(x_id,1, 2, ERROR_1, static_cast<long int>(1234));
...
test(multiple_entries) {
  ...
  logger.log(x_id,1, 1, SENSOR_HUMIDITY, static_cast<float>(3.14));
  logger.log(x_id,1, 2, ERROR_1, static_cast<long int>(1234));
  logger.log(x_id,2, 3, SENSOR_TEMPERATURE, static_cast<float>(6.28));
  logger.log(x_id,2, 3, ERROR_2, static_cast<long int>(5678));
...
test(check_entries) {
  ...
  logger.log(x_id,1, 2, SENSOR_HUMIDITY, static_cast<float>(3.14));
  logger.log(x_id,1, 3, ERROR_1, static_cast<long int>(1234));

  LogEntry entry1 = logger.getLog(0);
  assertEqual(entry1.x_id, x_id);
...
test(iterate_entries) {
  ...
  for (uint8_t i = 0; i < 10; i++) {
    if (i % 2 == 0) {
      logger.log(x_id,i, i + 1, SENSOR_HUMIDITY, static_cast<float>(i * 0.1f));
    } else {
      logger.log(x_id,i, i + 1, ERROR_1, static_cast<long int>(i * 100));
    }
  }
